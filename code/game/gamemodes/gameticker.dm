	...
	proc/equip_characters()
		...
		for(var/mob/living/carbon/human/player in player_list)
			if(player && player.mind && player.mind.assigned_role)
				if(player.mind.assigned_role == "Statthalter")
					captainless=0
...
/datum/controller/gameticker/proc/declare_completion()
	...
	to_world("<br><br><br><H1>Раунд [mode.name] окончен!</H1>")
			...
			if(Player.stat != DEAD)
				...
				if(evacuation_controller.round_over() && evacuation_controller.emergency_evacuation)
					if(isNotAdminLevel(playerTurf.z))
						to_chat(Player, "<font color='blue'><b>Вы выжили, но остались на станции [station_name()], как [Player.real_name]...</b></font>")
					else
						to_chat(Player, "<font color='green'><b>Вы выжили и добрались до дома, как [Player.real_name].</b></font>")
				else if(isAdminLevel(playerTurf.z))
					to_chat(Player, "<font color='green'><b>Вы успешно прошли пересемнку на станции [station_name()], как [Player.real_name].</b></font>")
				else if(issilicon(Player))
					to_chat(Player, "<font color='green'><b>Вы остались функционировать на станции [station_name()], как [Player.real_name].</b></font>")
				else
					to_chat(Player, "<font color='blue'><b>Прошел очередной рабочий на станции [station_name()]. Вы были [Player.real_name].</b></font>")
			else
				if(isghost(Player))
					var/mob/observer/ghost/O = Player
					if(!O.started_as_observer)
						to_chat(Player, "<font color='red'><b>Вы не выжили на станции [station_name()]...</b></font>")
				else
					to_chat(Player, "<font color='red'><b>Вы не выжили на станции [station_name()]...</b></font>")
	to_world("<br>")
	...
	for (var/mob/living/silicon/ai/aiPlayer in mob_list)
		if (aiPlayer.stat != 2)
			to_world("<b>[aiPlayer.name] (Игрок: [aiPlayer.key]). Его законы на конец раунда:</b>")

		else
			to_world("<b>[aiPlayer.name] (Игрок: [aiPlayer.key]). Его законы перед отключением:</b>")

		aiPlayer.show_laws(1)

		if (aiPlayer.connected_robots.len)
			var/robolist = "<b>Подчиненные синтетики:</b> "
	...
	for (var/mob/living/silicon/robot/robo in mob_list)
		...
		if (!robo.connected_ai)
			if (robo.stat != 2)
				to_world("<b>[robo.name] (Игрок: [robo.key]). Выжил, как независимый синтететик! Законы:</b>")

			else
				to_world("<b>[robo.name] (Played by: [robo.key]). Не выжил без ИИ. Его законы:</b>")
	...
	if(dronecount)
		to_world("<b>У нас [dronecount>1 ? "был" : "были"] [dronecount] [dronecount>1 ? "индустриальный дрон" : "индустриальных дрона"] на конец раунда.</b>")
	...
	*/
	...
	//ROUND END STATS
	var/round_end_stats = "<b>СТАТИСТИКА:</b>\n"
	round_end_stats += "Столько чести потер&#255;ли: <font color='red'><B>[honor_today]</B></font>.\n"
	round_end_stats += "Столько раз насрали на пол: <font color='red'><B>[shit_left]</B></font>.\n"
	round_end_stats += "Столько раз нассали на пол: <font color='red'><B>[piss_left]</B></font>.\n"
	round_end_stats += "Столько оргазмов испытал экипаж станции: <font color='red'><B>[times_came]</B></font>.\n"
	round_end_stats += "Столько умерло в космосе: <font color='red'><B>[deaths_in_space]</B></font>.\n"
	round_end_stats += "Общее количество выбитых зубов: <font color='red'><B>[teeth_lost]</B></font>.\n"
	round_end_stats += "Общее количество смертей: <font color='red'><B>[total_deaths]</B></font>.\n"
	round_end_stats += "<font color='red'>Последователи коммунизма:</font>\n"
	for(var/mob/living/carbon/human/H in human_mob_list)
		if(H.religion == IDEOLOGY_COMMUNISM)
			round_end_stats += "<font color='red'><b>[H.name]</b></font>\n"
	round_end_stats += "<font color='red'>Последователи свободного рынка:</font>\n"
	for(var/mob/living/carbon/human/H in human_mob_list)
		if(H.religion == IDEOLOGY_LIBERT)
			round_end_stats += "<font color='red'><b>[H.name]</b></font>\n"
	round_end_stats += "<font color='red'>Последователи фашизма:</font>\n"
	for(var/mob/living/carbon/human/H in human_mob_list)
		if(H.religion == IDEOLOGY_FASCISM)
			round_end_stats += "<font color='red'><b>[H.name]</b></font>\n"
	for(var/mob/living/carbon/human/H in human_mob_list)
		if(H.true_branded)
			true_branded_guys += 1
			if(true_branded_guys == 2)
				true_branded_guys = 0
				for(var/datum/mind/HG)
					if(HG.assigned_role == ("Offizier der Geheimpolizei" || "Direktor der Geheimpolizei"))
						if(HG.current)
							if(HG.current.client)
...
/datum/controller/gameticker/proc/create_event_of_round()
	var/event = pick(typesof(/datum/event_of_round) - /datum/event_of_round)
	eof = new event
	eof.apply_event()
	eof.announce_event()

/datum/controller/gameticker/proc/choose_ideologies_leaders()
	for(var/datum/ideology/I)
		I.choose_leader()


	var/ideology_message = ""
