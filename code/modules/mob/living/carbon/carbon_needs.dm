/mob/living/carbon/proc/print_happiness()
	var/msg = "\n<span class='info'>Мо&#255; идеологи&#255;: <font color='red'>[religion]</font></span>.\n"
	msg += "<span class='info'>Я [get_social_class()]</span>\n"
	msg += "<span class='info'>*---------*\nМоё настроение\n"
	for(var/i in events)
	...
	if(!events.len)
		msg += "<span class='info'>Всё как обычно.</span>\n"
...
/mob/living/carbon/proc/update_happiness()
	...
	if(old_icon && old_icon != happiness_icon.icon_state)
		if(old_happiness > happiness)
			var/badmood_text = pick("Моё настроение ухудшилось..", "Моё настроение ухудшилось!")
			to_chat(src, " <span class='warning'>[badmood_text]</span>")
		else
			var/happiness_text = pick("Моё настроение улучшилось.", "Моё настроение улучшилось!")
			to_chat(src, " <span class='info'>[happiness_text]</span>")
