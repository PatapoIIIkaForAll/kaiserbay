...
/*	Uncross(atom/movable/A)
	...
	Uncross(atom/movable/AM)
		if(AM.dir == dir)
			// This is hackish but whatever.
			var/turf/target = get_step(GetAbove(AM), dir)
			var/turf/source = AM.loc
			var/turf/above = GetAbove(AM)
			var/oldpos = AM.loc
			if(above.CanZPass(source, UP) && target.Enter(AM, source))
				if(ismob(AM))
					var/mob/M = AM
					if(M.pulling)
				...
				to_chat(AM, "<span class='warning'>Something blocks the path.</span>")
			return 0
		return 1
